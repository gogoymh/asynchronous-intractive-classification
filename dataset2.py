...
class Pseudo_Labeled_MNIST(Dataset):
    def __init__(self, capacity, transform):
        ...
        self.mnist = self.mnist.astype('uint8')
        self.label = np.empty((self.capacity))
    ...
    def labeling(self, img, label, real_index):
        if self.full:
            self.indicies[self.index] = real_index
        else:
            self.indicies.append(real_index)
    ...
    def check(self, real_index):
        if real_index in self.indicies:
            print("pseudo label is revised")
            index = self.indicies.index(real_index)
            self.indicies.remove(real_index)
            self.mnist = np.delete(self.mnist, index, axis=0)
            self.label = np.delete(self.label, index, axis=0)
            
            mnist_tmp = np.empty((1,28,28,1)).astype('uint8')
            self.mnist = np.concatenate((self.mnist, mnist_tmp), axis=0)
            
            label_tmp = np.empty((1))
            self.label = np.concatenate((self.label, label_tmp), axis=0)
...
class Unlabeled_MNIST(Dataset):
    ...
    def show(self, abs_index, similarity):
        img = self.mnist[abs_index]
        img = img.squeeze()
        plt.imshow(img, cmap='gray')
        plt.savefig("./images/index_%07d_%f.png" % (abs_index, similarity))
        plt.show()
        plt.close()
...
if __name__ == "__main__":    
    ...
    c = Pseudo_Labeled_MNIST(1, transform)
