...
def label(fileno, feature, represent, classifier, dataset2_list, index_list, measure1, measure2, device, test_loader):
    ...
    while len(unlabeled_index) > 1:
        ...
        _, similar_index = measure1.which(a_r,b)
        ...
        #_, filtered_index = measure2.which(a_c, output)
        
        pred = output.argmax(dim=1)
        filtered_index = pred == label
        ...
        if len(pseudo_index) > 500:
            howmany = len(pseudo_index) - 500
    ...
    while len(pseudo_index) > 1:
        ...
        pseudo_sampler = SubsetRandomSampler(pseudo_index)
        self_size = min(len(pseudo_index), 500)
        ...
        _, similar_index = measure1.which(a,b)
        similar_index = similar_index + 1
